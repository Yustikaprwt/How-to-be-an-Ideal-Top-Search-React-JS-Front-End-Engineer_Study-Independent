...
`;
const SubscriptionPengunjung = gql`
...
function App() {
  ...
  const { data: dataSubscription, loading: loadingSubscription, error} = useSubscription(SubscriptionPengunjung);

  if (error) return <h2>Error!{console.error(error)}</h2>;
  ...
  console.info("dataku",dataSubscription);
